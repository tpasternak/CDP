...
constexpr tuple<double, double, double>
get3dCoordinates(ptree::value_type const &entry) {
  const double x = entry.second.get<double>("3d-X");
  const double y = entry.second.get<double>("3d-Y");
  const double z = entry.second.get<double>("3d-Z");
